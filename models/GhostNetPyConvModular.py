...
import math
from attention_mechs.ScaleAttentionLight import PyConv
...
class GhostBlock(nn.Module):
    def __init__(self, inp, hidden_dim, oup, kernel_size, stride, use_se, use_pyconv, att_type_se, att_type_pyconv):
        ...
        # Squeeze-and-Excite
        if use_se == 1:
            new_layers.append(att_type_se(hidden_dim))

        # PyConv
        if use_pyconv == 1:
            new_layers.append(att_type_pyconv(hidden_dim))
...
class GhostNet(nn.Module):
    def __init__(self, cfgs, num_classes=1000, width_mult=1, att_type=SELayer):
        ...
        for k, exp_size, c, use_se, s, use_pyconv in self.cfgs:
            output_channel = _make_divisible(c * width_mult, 4)
            hidden_channel = _make_divisible(exp_size * width_mult, 4)
            
            layers.append(GhostBlock(input_channel, hidden_channel, output_channel, k, s, use_se, use_pyconv, SELayer, PyConv))
            input_channel = output_channel
