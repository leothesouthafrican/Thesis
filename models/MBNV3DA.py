...
from DANet import SAM as SAM_Module
...
class InvertedResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernal_size, stride, NL, SAM, exp_size):
        ...
        if self.SAM:
            self.SAM_Module = SAM_Module(in_channels, heads=1)
...
class MobileNetV3DANet(nn.Module):
    def __init__(self, mode, num_classes=10, mu=1.0, dropout=0.2):
        super(MobileNetV3DANet, self).__init__()
        self.num_classes = num_classes

        if mode == 'small':
            self.cfg = [
                #in, out, k, s, nl, DANet,  exp
            ...
            self.layers = []
            for i, (in_channels, out_channels, kernal_size, stride, NL, SAM_True, exp_size) in enumerate(self.cfg):
                ...
                self.layers.append(InvertedResidualBlock(in_channels, out_channels, kernal_size, stride, NL, SAM_True, exp_size))
...
if __name__ == "__main__":
    model = MobileNetV3DANet(mode='large')
